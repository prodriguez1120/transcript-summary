                ...
                if result:
                    ...
                    if "ranked_quotes" in result:
                        ...
                        if ranked_quotes:
                            ...
                            print(
                                f"    OpenAI Rank: {top_quote.get('openai_rank', 'N/A')}"
                            )
                            print(
                                f"    Selection Stage: {top_quote.get('selection_stage', 'N/A')}"
                            )
                            print(
                                f"    Relevance Explanation: {top_quote.get('relevance_explanation', 'N/A')[:50]}..."
                            )
                    ...
                    if themes:
                        ...
                        for i, theme in enumerate(themes[:2]):
                            ...
                            if theme_quotes:
                                ...
                                print(
                                    f"      Top quote rank: {top_theme_quote.get('openai_rank', 'N/A')}"
                                )
                                print(
                                    f"      Selection stage: {top_theme_quote.get('selection_stage', 'N/A')}"
                                )
                else:
        ...
        try:
            ...
            if "business_model" in analyzer.key_perspectives:
                ...
                if actual_result:
                    ...
                    ranking_stats = analyzer.get_quote_ranking_statistics(
                        actual_results
                    )
                    print(f"  📈 Actual Ranking Statistics:")
                    print(
                        f"    Total Perspectives: {ranking_stats['total_perspectives']}"
                    )
                    print(
                        f"    Total Quotes Ranked: {ranking_stats['total_ranked_quotes']}"
                    )
                    print(
                        f"    Ranking Coverage: {ranking_stats['ranking_coverage']:.1f}%"
                    )

                    if ranking_stats["selection_stage_breakdown"]:
                        print(f"    Selection Stage Breakdown:")
                        for stage, count in ranking_stats[
                    ...
                    if "ranked_quotes" in actual_result:
                        ...
                        for rank in sorted(ranking_distribution.keys()):
                            print(
                                f"      Rank {rank}: {ranking_distribution[rank]} quotes"
                            )
                else:
