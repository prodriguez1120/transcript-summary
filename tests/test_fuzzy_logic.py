...
class TestFuzzyMatchingLogic(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.matcher = FuzzyMatcher(
            use_semantic=False, fuzzy_threshold=80, semantic_threshold=0.7
        )
    ...
    def test_exact_pattern_matching(self):
        ...
        is_match, confidence, pattern = self.matcher.fuzzy_topic_match(
    ...
    def test_fuzzy_string_matching(self):
        ...
        is_match, confidence, pattern = low_threshold_matcher.fuzzy_topic_match(
            text, self.topic_patterns
        )
    ...
    def test_semantic_matching_fallback(self):
        ...
        with patch("fuzzy_matching.SentenceTransformer") as mock_transformer:
            ...
            semantic_matcher = FuzzyMatcher(
                use_semantic=True, fuzzy_threshold=95, semantic_threshold=0.6
            )

            text = "FlexXray's competitive positioning in the marketplace"
            patterns = ["market leadership", "competitive advantage"]

            is_match, confidence, pattern = semantic_matcher.fuzzy_topic_match(
                text, patterns
            )
    ...
    def test_pattern_variations_handling(self):
        ...
        self.assertTrue(
            "local" in pattern.lower()
            or "regional" in pattern.lower()
            or "footprint" in pattern.lower()
        )
    ...
    def test_semantic_similarity_calculation(self):
        ...
        with patch("fuzzy_matching.SentenceTransformer") as mock_transformer:
            ...
            )  # Should match highest similarity
