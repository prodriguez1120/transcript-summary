...
def test_prompt_validation():
    ...
    # Test valid prompt
    assert config.validate_prompt(
        "quote_ranking"
    ), "Valid prompt should pass validation"

    # Test invalid prompt
    assert not config.validate_prompt(
        "nonexistent"
...
def test_prompt_updates():
    ...
    success = config.update_prompt(
        "quote_ranking", template="Test template", system_message="Test system message"
    )
...
def test_all_prompt_types():
    ...
    for expected_type in expected_types:
        assert (
            expected_type in prompt_types
        ), f"Prompt type {expected_type} should be available"
        ...
        ), f"System message for {expected_type} should be available"
...
def test_template_variables():
    ...
    try:
        formatted = config.format_prompt("quote_ranking", perspective_title="Test")
