...
def test_fuzzy_topic_matching():
    ...
    topics = {
        ...
        'local_presence': ['local presence', 'regional footprint', 'geographic coverage'],
...
def test_fuzzy_speaker_identification():
    ...
    interviewer_patterns = [
        "tell me about", "can you elaborate", "what do you think",
        "how do you handle", "that's interesting", "explain more"
    ]
    
    expert_patterns = [
        "we have been", "our customers", "we provide", "we focus",
        "our technology", "our proprietary", "our local presence"
    ]
...
def test_fuzzy_insight_detection():
    ...
    insight_patterns = [
        "competitive advantage", "technology", "value", "market growth",
        "customer satisfaction", "improve", "revenue", "business"
    ]
    
    for i, sentence in enumerate(test_sentences):
        has_insight, confidence, matched_patterns = fuzzy_matcher.fuzzy_insight_detection(
            sentence, insight_patterns
        )
...
def test_enhanced_quote_filtering():
    ...
    # Test quotes
    test_quotes = [
        {"text": "We are the market leader in foreign material inspection services.", "speaker_role": "expert"},
        {"text": "Our company dominates the industry with proprietary technology.", "speaker_role": "expert"},
        {"text": "We have a strong value proposition that customers appreciate.", "speaker_role": "expert"},
        {"text": "The local presence gives us a competitive advantage.", "speaker_role": "expert"},
        {"text": "Our turnaround times are incredibly fast.", "speaker_role": "expert"},
        {"text": "The market size is somewhat limited for our services.", "speaker_role": "expert"},
        {"text": "Timing can be unpredictable in this business.", "speaker_role": "expert"}
    ]
    
    # Test filtering by different topics
    topics = ['market_leadership', 'value_proposition', 'local_presence', 'technology_advantages']
    ...
    for topic in topics:
        ...
        for quote in filtered_quotes:
            ...
            if 'fuzzy_match' in quote:
                ...
                print(f"      Confidence: {match_info['confidence']:.1f}%, Method: {match_info['matching_method']}")
        print()
