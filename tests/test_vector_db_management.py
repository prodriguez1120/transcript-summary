...
def test_database_management():
    ...
    try:
        ...
        print("2. analyzer.clear_vector_database() - Clear all quotes (use with caution!)")
        print("3. analyzer.store_quotes_in_vector_db(quotes) - Store with duplicate detection")
...
def demonstrate_duplicate_scenarios():
    ...
    print("  - Output: 'All 100 quotes already exist in vector database - no new quotes to add'")
...
def main():
    ...
    if not os.getenv('OPENAI_API_KEY'):
        print("⚠️  OPENAI_API_KEY not set. Database functions will work, but analysis may be limited.")
