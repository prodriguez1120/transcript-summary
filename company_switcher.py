...
Examples:
  ...
  python company_switcher.py --validate                # Validate transcript directory
    ...
    parser.add_argument(
        '--switch', '-s',
        metavar='COMPANY',
        help='Switch to specified company'
    )
    
    parser.add_argument(
        '--info', '-i',
        action='store_true',
        help='Show current company information'
    )
...
def create_company_interactive(config_mgr: ConfigManager, company_name: str):
    ...
    display_name = input(f"Display name [{company_name.title()}]: ").strip() or company_name.title()
    transcript_dir = input(f"Transcript directory [{display_name} Transcripts]: ").strip() or f"{display_name} Transcripts"
    output_prefix = input(f"Output prefix [{display_name.replace(' ', '')}]: ").strip() or display_name.replace(' ', '')
    ...
    while True:
        question_key = input(f"Question {question_num} key (e.g., market_position): ").strip()
        if not question_key:
            break
    ...
    if not question_categories:
        ...
        question_categories = {
            ...
            "strengths": list(key_questions.keys())[2:] if len(key_questions) > 2 else []
        }
...
def validate_transcript_directory(config_mgr: ConfigManager):
    ...
    if config_mgr.validate_transcript_directory():
        ...
        print(f"✗ Transcript directory does not exist: {config_mgr.get_transcript_directory()}")
