...
class JSONExtractor:
    ...
    def _extract_balanced_json(text: str) -> Optional[str]:
        ...
        for start_char in start_chars:
            ...
            if start_pos != -1:
                ...
                json_text = JSONExtractor._extract_balanced_content(
                    text, start_pos, start_char, end_char
                )
    ...
    def _extract_balanced_content(
        text: str, start_pos: int, start_char: str, end_char: str
    ) -> Optional[str]:
    ...
    def _apply_common_fixes(text: str) -> str:
        ...
        for start_char in ["{", "["]:
            if start_char in fixed:
                end_char = "}" if start_char == "{" else "]"
                balanced = JSONExtractor._extract_balanced_content(
                    fixed, 0, start_char, end_char
                )
                if balanced:
                    return balanced
...
class JSONParser:
    ...
    def parse_json_safe(json_text: str, context: str = "JSON parsing") -> Any:
        ...
        try:
            ...
            raise JSONParsingError(
                f"Unexpected error parsing JSON in {context}", str(e), e
            )

    @staticmethod
    def validate_json_structure(
        data: Any, expected_type: type, context: str = "JSON validation"
    ) -> Any:
    ...
    @staticmethod
    def extract_and_validate_quotes_ranking(
        response_text: str, quotes: List[Dict[str, Any]]
    ) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            for i, ranking in enumerate(rankings):
                ...
                if 0 <= quote_index < len(quotes):
                    validated_ranking = {
                        ...
                        "relevance_explanation": ranking.get(
            ...
            raise JSONParsingError(
                "Failed to extract and validate quote rankings", str(e), e
            )
    ...
    def extract_and_validate_themes(response_text: str) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            raise JSONParsingError("Failed to extract and validate themes", str(e), e)
