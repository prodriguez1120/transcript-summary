...
class PerspectiveAnalyzer:
    def __init__(self, api_key: str):
        ...
        self.max_quotes_per_perspective = 200  # Maximum quotes to process per perspective
    ...
    def configure_batch_processing(self, batch_size: Optional[int] = None, batch_delay: Optional[float] = None, 
                                 failure_delay: Optional[float] = None, max_quotes: Optional[int] = None,
        ...
        if batch_delay is not None:
            self.batch_delay = max(0.5, min(5.0, batch_delay))  # Ensure reasonable range
            print(f"Batch delay set to {self.batch_delay}s")
        
        if failure_delay is not None:
            self.failure_delay = max(1.0, min(10.0, failure_delay))  # Ensure reasonable range
            print(f"Failure delay set to {self.failure_delay}s")
        
        if max_quotes is not None:
            self.max_quotes_per_perspective = max(50, min(500, max_quotes))  # Ensure reasonable range
            print(f"Max quotes per perspective set to {self.max_quotes_per_perspective}")
        ...
        print(f"Batch processing configuration: size={self.batch_size}, delay={self.batch_delay}s, failure_delay={self.failure_delay}s, max_quotes={self.max_quotes_per_perspective}")
    ...
    def analyze_perspective_with_quotes(self, perspective_key: str, perspective_data: dict,
        ...
        return {
            ...
            'total_ranked_quotes': len([q for q in ranked_quotes if q.get('selection_stage') in ['openai_ranked', 'openai_failed', 'parsing_failed', 'batch_failed']]),
            'batch_processing_stats': self._get_batch_processing_stats(ranked_quotes)
        }

    def _get_batch_processing_stats(self, ranked_quotes: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        for quote in ranked_quotes:
            ...
            stats['selection_stages'][stage] = stats['selection_stages'].get(stage, 0) + 1
        ...
        if stats['total_quotes'] > 0:
            stats['coverage_percentage'] = (stats['successful_rankings'] / stats['total_quotes']) * 100
    ...
    def get_batch_processing_metrics(self) -> Dict[str, Any]:
        ...
        metrics = {
            ...
            'performance': {
                'estimated_quotes_per_minute': int(60 / (self.batch_delay + 0.5)),  # Rough estimate
                'estimated_batch_processing_time': lambda quote_count: (quote_count / self.batch_size) * (self.batch_delay + 0.5),
    ...
    def _expand_focus_areas(self, focus_areas: List[str]) -> List[str]:
        ...
        focus_expansions = {
            'market': ['market', 'marketplace', 'market demand', 'market size', 'market share', 'market position'],
            'customer': ['customer', 'client', 'user', 'buyer', 'consumer', 'customer satisfaction', 'customer needs'],
            'technology': ['technology', 'tech', 'innovation', 'digital', 'software', 'platform', 'solution'],
            'business': ['business', 'company', 'organization', 'enterprise', 'operation', 'strategy'],
            'growth': ['growth', 'expansion', 'scaling', 'development', 'increase', 'improvement'],
            'competition': ['competition', 'competitive', 'competitor', 'rival', 'market leader', 'competitive advantage'],
            'revenue': ['revenue', 'sales', 'income', 'profit', 'earnings', 'financial performance'],
            'quality': ['quality', 'excellence', 'standards', 'performance', 'efficiency', 'effectiveness']
        }
        ...
        for focus_area in focus_areas:
            ...
            for key, related_terms in focus_expansions.items():
                if key in focus_lower or any(term in focus_lower for term in key.split()):
                    # Add related terms that aren't already in the focus areas
                    for term in related_terms:
                        if term not in expanded_areas and not any(term in existing.lower() for existing in expanded_areas):
                            expanded_areas.append(term)
        
        print(f"Expanded focus areas from {len(focus_areas)} to {len(expanded_areas)} for better coverage")
    ...
    def _find_relevant_quotes_for_perspective(self, perspective_key: str, perspective_data: dict,
        ...
        try:
            ...
            if hasattr(self, 'vector_db_manager') and self.vector_db_manager and self.vector_db_manager.quotes_collection:
                ...
                for focus_area in expanded_focus_areas:
                    ...
                    search_results = self.vector_db_manager.semantic_search_quotes(
                        ...
                        filter_metadata={'speaker_role': 'expert'}  # Only expert quotes
                    )
                    ...
                    for result in search_results:
                        ...
                        if any(q.get('id') == quote_id or q.get('text') == quote_text for q in relevant_quotes):
                            continue
                        ...
                        quote = {
                            'text': quote_text,
                            'speaker_role': 'expert',
                            'transcript_name': result.get('metadata', {}).get('transcript_name', 'Unknown'),
                ...
                print(f"Vector database search found {len(relevant_quotes)} relevant quotes")
                
            else:
                # Fallback to local filtering if vector database not available
                print(f"Vector database not available, using local filtering for {perspective_key}...")
        ...
        print(f"Processing top {min(len(relevant_quotes), max_quotes)} quotes out of {len(relevant_quotes)} relevant quotes")
    ...
    def _calculate_focus_area_relevance(self, quote_text: str, focus_area: str) -> float:
        ...
        focus_words = [word.strip() for word in focus_lower.split() if len(word.strip()) > 2]
        
        # Count exact matches
        exact_matches = sum(1 for word in focus_words if word in quote_lower)
        
        # Count partial matches (for compound terms)
        partial_matches = sum(1 for word in focus_words if any(word in quote_word for quote_word in quote_lower.split()))
        
        # Calculate base score - made less restrictive for better coverage
        base_score = (exact_matches * 1.5) + (partial_matches * 0.8)  # Reduced from 2.0/1.0
        ...
        business_terms = ['business', 'market', 'customer', 'strategy', 'growth', 'revenue', 'profit', 'competition', 'industry']
    ...
    def _fallback_local_filtering(self, all_quotes: List[Dict[str, Any]], focus_areas: List[str]) -> List[Dict[str, Any]]:
    ...
    def _rank_quotes_with_openai_batch(self, perspective_key: str, perspective_data: dict,
                                     quotes: List[Dict[str, Any]], batch_size: Optional[int] = None) -> List[Dict[str, Any]]:
        ...
        print(f"Starting batch processing for {len(quotes)} quotes with batch size {batch_size}")
        ...
        for i in range(0, len(quotes), batch_size):
            ...
            try:
                # Process this batch
                ranked_batch = self._rank_quotes_with_openai_single(perspective_key, perspective_data, batch)
                
                if ranked_batch:
                    all_ranked_quotes.extend(ranked_batch)
                    print(f"✅ Batch {batch_num} completed successfully - {len(ranked_batch)} quotes ranked")
                else:
        ...
        print(f"Batch processing completed: {len(all_ranked_quotes)} total quotes processed")
    ...
    def _rank_quotes_with_openai_single(self, perspective_key: str, perspective_data: dict,
                                       quotes: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            if not system_message:
                ...
                system_message = "You are an expert analyst. Respond with ONLY valid JSON."
...
RESPOND WITH ONLY THE JSON ARRAY. NO OTHER TEXT. NO GREETINGS. NO EXPLANATIONS."""
                ...
                fallback_response = self.client.chat.completions.create(
                    model=params.get("model", "gpt-4"),
                    messages=[
                        {"role": "system", "content": "You are an AI assistant that ONLY responds with valid JSON. Never include conversational text."},
...
Return: [{{"quote_index": 1, "relevance_score": 8, "relevance_explanation": "relevant", "key_insight": "insight"}}]"""
                    ...
                    minimal_response = self.client.chat.completions.create(
                        model=params.get("model", "gpt-4"),
                        messages=[
                            {"role": "system", "content": "JSON only. No conversation."},
                    ...
                    print(f"Second fallback response preview: {minimal_content[:100]}...")
                    
                    ranked_quotes = self._parse_ranking_response(minimal_content, quotes)
                    print(f"Second fallback successful - ranked {len(ranked_quotes)} quotes")
                    return ranked_quotes
    ...
    def _parse_ranking_response(self, response_text: str, quotes: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            print(f"  - Processed through ranking pipeline: {len(quotes) - len(explicitly_ranked_indices)}")
    ...
    def _select_quotes_for_theme(self, theme_name: str, quotes: List[Dict[str, Any]], max_quotes: int) -> List[Dict[str, Any]]:
