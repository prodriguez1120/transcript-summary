...
# Custom Exceptions
class VectorDatabaseError(Exception): pass
class ChromaDBConnectionError(VectorDatabaseError): pass
class QuoteStorageError(VectorDatabaseError): pass
class SearchError(VectorDatabaseError): pass
class DataValidationError(VectorDatabaseError): pass
...
class VectorDatabaseManager:
    def __init__(
        ...
        if not CHROMA_AVAILABLE:
            self.logger.warning("ChromaDB not available. Vector DB operations will be limited.")
    ...
    def _embedding_function(self, texts: List[str]) -> List[List[float]]:
        ...
        try:
            ...
            self.logger.debug(f"Generated {len(embeddings)} embeddings with dimensions {len(embeddings[0]) if embeddings else 0}")
            return embeddings
    ...
    def store_quotes_in_vector_db(self, quotes: List[Dict[str, Any]], batch_size: int = 100) -> bool:
        ...
        try:
            ...
            self.logger.info(f"✅ Successfully stored {len(quotes)} quotes with OpenAI embeddings in {total_duration:.3f}s")
            return True
    ...
    def semantic_search_quotes(self, query: str, n_results: int = 10, filter_metadata: Dict = None) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            self.logger.debug(f"Search completed in {search_duration:.3f}s, found {len(quotes)} results")
            return quotes
    ...
    def verify_embeddings(self, sample_size: int = 3) -> Dict[str, Any]:
        ...
        try:
            ...
            self.logger.info("✅ OpenAI embedding verification complete - all documents have embeddings")
