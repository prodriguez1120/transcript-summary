...
def test_main_tool_rag():
    ...
    try:
        ...
        rag_enabled = (hasattr(analyzer.perspective_analyzer, 'vector_db_manager') and 
                      analyzer.perspective_analyzer.vector_db_manager is not None)
        print(f"  RAG Integration: {'✅ Enabled' if rag_enabled else '❌ Disabled'}")
        
        if rag_enabled:
            print(f"  Vector DB Manager: {type(analyzer.perspective_analyzer.vector_db_manager).__name__}")
            print(f"  Quotes Collection: {'✅ Available' if analyzer.perspective_analyzer.vector_db_manager.quotes_collection else '❌ Not Available'}")
            ...
            try:
                ...
                if result:
                    ...
                    if result.get('total_quotes', 0) > len(test_quotes):
                        print(f"  🔍 RAG detected: Retrieved {result.get('total_quotes', 0)} quotes from vector DB")
                    else:
                        print(f"  📝 Local processing: Used {len(test_quotes)} provided quotes")
                else:
