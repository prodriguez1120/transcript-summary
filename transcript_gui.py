...
class TranscriptAnalyzerGUI:
    ...
    def setup_ui(self):
        ...
        # Title
        title_label = ttk.Label(main_frame, text="Optimized Transcript Summarizer", font=("Arial", 16, "bold"))
        ...
        # Directory Selection
        ttk.Label(main_frame, text="Transcript Directory:").grid(row=2, column=0, sticky=tk.W, pady=5)
        ...
        performance_frame.grid(row=3, column=0, columnspan=3, sticky=(tk.W, tk.E), pady=10)
        performance_frame.columnconfigure(1, weight=1)
        
        ttk.Label(performance_frame, text="Max Concurrent API Calls:").grid(row=0, column=0, sticky=tk.W, pady=5)
        workers_spinbox = ttk.Spinbox(performance_frame, from_=1, to=5, textvariable=self.max_workers_var, width=10)
        workers_spinbox.grid(row=0, column=1, sticky=tk.W, pady=5, padx=(5, 0))
        ttk.Label(performance_frame, text="(Higher = faster but may hit rate limits)").grid(row=0, column=2, sticky=tk.W, pady=5, padx=(5, 0))
        ...
        for section_key, section_data in temp_analyzer.questions.items():
            ...
            cb = ttk.Checkbutton(options_frame, text=section_data['title'], variable=var)
        ...
        self.analyze_btn = ttk.Button(button_frame, text="Start Optimized Analysis", command=self.start_analysis)
        self.analyze_btn.pack(side=tk.LEFT, padx=(0, 10))
        
        self.view_results_btn = ttk.Button(button_frame, text="View Results", command=self.view_results, state=tk.DISABLED)
    ...
    def browse_directory(self):
        directory = filedialog.askdirectory(initialdir="FlexXray Transcripts" if os.path.exists("FlexXray Transcripts") else ".")
        if directory:
    ...
    def update_progress(self, message, progress):
        ...
        if self.start_time:
            ...
            self.root.after(0, lambda: self.time_label.config(text=f"Time elapsed: {elapsed:.1f} seconds"))
    ...
    def start_analysis(self):
        ...
        if not self.directory_path.get().strip():
            messagebox.showerror("Error", "Please select a directory containing Word document transcripts.")
            return
        ...
        if not selected_sections:
            messagebox.showerror("Error", "Please select at least one analysis section.")
            return
        ...
        thread = threading.Thread(target=self.run_optimized_analysis, args=(selected_sections,))
        thread.daemon = True
        thread.start()
    ...
    def run_optimized_analysis(self, selected_sections):
        try:
            ...
            temp_analyzer = TranscriptSummarizer("temp")
            analyzer.questions = {key: temp_analyzer.questions[key] for key in selected_sections}
            
            self.log_message(f"Processing transcripts in: {self.directory_path.get()}")
            self.log_message(f"Using {self.max_workers_var.get()} concurrent API calls for optimal performance")
            ...
            if results:
                ...
                self.log_message(f"Optimized analysis complete! Processed {len(results)} transcripts in {total_time:.2f} seconds.")
                self.log_message("Files created:")
                if CONFIG_AVAILABLE:
                    self.log_message(f"- {OUTPUT_FILES['transcript_analysis_json']} (structured data)")
                    self.log_message(f"- {OUTPUT_FILES['transcript_analysis_txt']} (human-readable report)")
                    self.log_message(f"- {OUTPUT_FILES['transcript_analysis_docx']} (optimized Word document)")
                    self.log_message(f"All files have been saved to: {get_output_path('')}")
                else:
                    self.log_message("- FlexXray_transcript_analysis_results.json (structured data)")
                    self.log_message("- FlexXray_transcript_analysis_results.txt (human-readable report)")
                    self.log_message("- FlexXray_transcript_analysis_results.docx (optimized Word document)")
                    self.log_message("All files have been saved to the current directory.")
                
                # Enable view results button
                self.root.after(0, lambda: self.view_results_btn.config(state=tk.NORMAL))
                self.root.after(0, lambda: self.status_var.set(f"Analysis complete! ({total_time:.2f}s)"))
                self.root.after(0, lambda: self.progress_var.set(100))
            else:
                self.log_message("No transcripts were successfully processed.")
                self.root.after(0, lambda: self.status_var.set("No transcripts processed"))
    ...
    def view_results(self):
        ...
        if os.path.exists(word_file):
            ...
            messagebox.showinfo("Info", "No results file found. Please run the analysis first.")
