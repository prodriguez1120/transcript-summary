...
class FuzzyMatcher:
    ...
    def __init__(
        ...
        if self.use_semantic:
            try:
                ...
                logger.warning(
                    f"Failed to load semantic model: {e}. Falling back to fuzzy string matching only."
                )
                self.use_semantic = False
                self.semantic_model = None
            except Exception as e:
                logger.warning(
                    f"Unexpected error loading semantic model: {e}. Falling back to fuzzy string matching only."
                )
                self.use_semantic = False
                self.semantic_model = None
        else:
    ...
    def fuzzy_topic_match(
        self, text: str, topic_patterns: List[str], use_semantic: bool = None
        ...
        for pattern in topic_patterns:
            if re.search(pattern, text_lower, re.IGNORECASE):
                logger.debug(
                    f"REGEX MATCH: pattern='{pattern}' matched text='{text_lower}'"
                )
                return True, 100.0, pattern

        # Try fuzzy string matching
        best_fuzzy_match = process.extractOne(
            text_lower, topic_patterns, scorer=fuzz.partial_ratio
        )
        ...
        if use_semantic and self.semantic_model:
            try:
                semantic_score, best_semantic_pattern = self._semantic_topic_match(
                    text, topic_patterns
                )
    ...
    def _semantic_topic_match(
        self, text: str, topic_patterns: List[str]
    ) -> Tuple[float, str]:
        ...
        try:
            ...
            for i, pattern_embedding in enumerate(pattern_embeddings):
                similarity = np.dot(text_embedding[0], pattern_embedding) / (
                    np.linalg.norm(text_embedding[0])
    ...
    def fuzzy_speaker_identification(
        self, sentence: str, interviewer_patterns: List[str], expert_patterns: List[str]
    ) -> Tuple[str, float]:
        """
    ...
    def fuzzy_insight_detection(
        self, sentence: str, insight_patterns: List[str]
        ...
        confidence = (
            min(total_score / len(insight_patterns) * 10, 100)
            if insight_patterns
            else 0
        )
    ...
    def enhanced_topic_filtering(
        self, quotes: List[Dict[str, Any]], topic: str, topic_patterns: List[str]
    ) -> List[Dict[str, Any]]:
        """
        ...
        for quote in quotes:
            ...
            is_match, confidence, best_pattern = self.fuzzy_topic_match(
                text, topic_patterns
            )
