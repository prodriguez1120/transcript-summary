...
class QuoteExtractor:
        ...
        try:
            ...
            if file_extension == ".docx":
                ...
                try:
                    ...
                    extracted_text = "\n\n".join(text_parts)
                    print(
                        f"Successfully extracted {len(extracted_text)} characters from {Path(doc_path).name}"
                    )
    ...
    def extract_quotes_from_text(
        ...
        max_context_sentences = (
            3  # Keep last 3 interviewer sentences as potential context
        )
        ...
        for i, sentence in enumerate(sentences):
            ...
            if (
                len(sentence) < self.min_quote_length
                or len(sentence) > self.max_quote_length
            ):
                continue
            ...
            if speaker_role == "interviewer":
                ...
                if len(recent_interviewer_context) > max_context_sentences:
                    recent_interviewer_context = recent_interviewer_context[
                        -max_context_sentences:
                    ]
            ...
            if re.match(
                r"^(So|Well|Um|Uh|Yeah|No|Yes|Right|Okay|I mean|You know|That\'s|It\'s)",
                sentence,
                re.IGNORECASE,
            ):
                continue
            ...
            if self.use_fuzzy and self.fuzzy_matcher:
                has_insight, confidence, matched_patterns = (
                    self.fuzzy_matcher.fuzzy_insight_detection(
            ...
            if has_insight:
                ...
                relevant_context = self._find_relevant_context(
                    sentence, recent_interviewer_context
                )
    ...
    def _is_question(self, sentence: str) -> bool:
        ...
        return any(
            re.search(pattern, sentence, re.IGNORECASE) for pattern in question_patterns
        )

    def _find_relevant_context(
        self, expert_quote: str, recent_context: List[Dict[str, Any]]
    ) -> List[Dict[str, Any]]:
