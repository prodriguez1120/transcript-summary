...
class TranscriptSummarizer:
    def __init__(self, api_key: str = None, max_workers: int = 3, chroma_persist_directory: str = "./chroma_db"):
    ...
    def semantic_chunk_text(self, text: str, transcript_name: str, doc_path: str = None) -> List[Dict[str, Any]]:
        """
        ...
        for segment in segments:
            ...
            if len(segment) > self.chunk_size:
                ...
                sub_chunks = self._split_long_segment(segment, transcript_name, chunk_id, doc_path)
        ...
        print(f"Created {len(chunks)} intelligent semantic chunks for {transcript_name}")
        return chunks
    ...
    def _split_long_segment(self, segment: str, transcript_name: str, base_chunk_id: int, doc_path: str) -> List[Dict[str, Any]]:
    ...
    def _create_chunk_metadata(self, text: str, transcript_name: str, chunk_id: int, doc_path: str, sentence_position: int) -> Dict[str, Any]:
        """Create metadata for a chunk including semantic context."""
        # Generate unique ID for the chunk
        chunk_hash = hashlib.md5(f"{transcript_name}_{chunk_id}_{text[:100]}".encode()).hexdigest()
    ...
    def _extract_key_topics(self, text: str) -> List[str]:
        ...
        key_terms = [
            "flexxray", "x-ray", "xray", "foreign material", "contamination",
            "food safety", "quality control", "inspection", "detection",
            "customer", "market", "industry", "growth", "revenue", "profit",
            "competition", "competitive", "advantage", "strength", "weakness",
            "risk", "opportunity", "technology", "innovation", "investment"
        ]
        ...
        for company in company_patterns[:3]:  # Limit to first 3 potential companies
            if len(company.split()) <= 3 and company.lower() not in ["the", "and", "for", "with"]:
    ...
    def _classify_chunk_type(self, text: str) -> str:
        ...
        financial_indicators = [
            "financial", "revenue", "profit", "cost", "pricing", "budget", "margin", "earnings",
            "cash flow", "investment", "funding", "valuation", "market cap", "ebitda",
            "quarterly", "annual", "growth rate", "market share", "pricing strategy"
        ]
        
        # Customer and market analysis
        customer_market_indicators = [
            "customer", "client", "market", "demand", "sales", "customer satisfaction",
            "market demand", "customer needs", "market trends", "customer relationships",
            "market size", "customer acquisition", "market penetration", "customer retention"
        ]
        
        # Technology and product development
        technology_indicators = [
            "technology", "innovation", "product", "development", "research", "r&d",
            "technical", "engineering", "software", "hardware", "platform", "solution",
            "product features", "technical capabilities", "product roadmap", "innovation"
        ]
        
        # Competitive analysis and positioning
        competitive_indicators = [
            "competition", "competitive", "advantage", "strength", "weakness", "differentiation",
            "market position", "competitive landscape", "competitive advantage", "market leader",
            "competitive analysis", "competitive pressure", "market positioning"
        ]
        
        # Risk and challenges
        risk_indicators = [
            "risk", "challenge", "problem", "issue", "concern", "threat", "obstacle",
            "regulatory", "compliance", "legal", "operational risk", "market risk",
            "competitive risk", "technology risk", "financial risk"
        ]
        
        # Growth and strategy
        growth_indicators = [
            "growth", "expansion", "opportunity", "future", "strategy", "strategic",
            "market expansion", "product development", "strategic initiatives", "growth strategy",
            "expansion plans", "market opportunity", "strategic positioning"
        ]
        
        # Industry and market trends
        industry_indicators = [
            "industry", "sector", "market trends", "industry trends", "regulatory changes",
            "market dynamics", "industry dynamics", "market evolution", "industry evolution"
        ]
        
        # Operations and processes
        operational_indicators = [
            "operations", "process", "efficiency", "quality", "supply chain", "logistics",
            "manufacturing", "production", "operational excellence", "process improvement"
        ]
        
        # Calculate scores for each category
        scores = {
            "financial": sum(1 for indicator in financial_indicators if indicator in text_lower),
            "customer_market": sum(1 for indicator in customer_market_indicators if indicator in text_lower),
            "technology_product": sum(1 for indicator in technology_indicators if indicator in text_lower),
            "competitive_analysis": sum(1 for indicator in competitive_indicators if indicator in text_lower),
            "risk_challenge": sum(1 for indicator in risk_indicators if indicator in text_lower),
            "growth_strategy": sum(1 for indicator in growth_indicators if indicator in text_lower),
            "industry_trends": sum(1 for indicator in industry_indicators if indicator in text_lower),
            "operational": sum(1 for indicator in operational_indicators if indicator in text_lower)
        }
    ...
    def search_relevant_chunks(self, query: str, transcript_name: str = None, top_k: int = 5) -> List[Dict[str, Any]]:
    ...
    def get_context_for_question(
        ...
        try:
            if use_hierarchical:
                ...
                relevant_chunks = self.search_relevant_chunks(query, transcript_name, top_k)
            ...
            for chunk in relevant_chunks:
                if isinstance(chunk, dict) and 'text' in chunk:
                    if 'rank' in chunk:
                        ...
                        context_parts.append(f"Context from {chunk['metadata'].get('transcript_name', 'Unknown')}:\n{chunk['text']}")
    ...
    def analyze_section(
        ...
        try:
            ...
            section_questions = "\n".join([f"{i+1}. {q}" for i, q in enumerate(section_data['questions'])])
...
"""
    ...
    def analyze_transcript(self, transcript_text: str, transcript_name: str) -> Dict[str, Any]:
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=self.max_workers) as executor:
            ...
            future_to_section = {
                executor.submit(self.analyze_section, transcript_text, section_key, section_data, transcript_name): section_key
    ...
    def process_transcripts_directory(self, directory_path: str, progress_callback=None) -> List[Dict[str, Any]]:
        ...
        for i, doc_file in enumerate(docx_files):
            if progress_callback:
                ...
                progress_callback(f"Processing {doc_file.name} ({i+1}/{total_files})", progress)
            ...
            chunks = self.semantic_chunk_text(transcript_text, doc_file.name, str(doc_file))
        ...
        try:
            ...
            for line in lines:
                ...
                if match:
                    ...
                    if name.lower() not in ['the company', 'the industry', 'the market', 'the business']:
                        return name
    ...
    def _extract_quotes_from_answer(self, answer_text: str) -> List[str]:
        ...
        try:
            ...
            for quote in quotes:
                quote = quote.strip()
                if quote and quote not in unique_quotes and len(quote) > 10:  # Minimum length to be meaningful
    ...
    def _is_information_unavailable(self, answer_text: str) -> bool:
        ...
        if len(answer_lower) < 50:  # Short answers are more likely to be "not available"
            for phrase in unavailable_phrases:
    ...
    def _parse_section_answers(self, answers: str, questions: List[str]) -> Dict[str, str]:
        ...
        section_headers = re.findall(r'(?:####|###)\s*(\d+)\.\s*Section\s+(\d+[a-f]):', answers)
        if section_headers:
                    ...
                    for i, section_num in enumerate(section_matches):
                        if i + 1 < len(parts):
                            ...
                            if content:
                                ...
                                next_section_match = re.search(r'Section\s+\d+[a-f]:', content)
        ...
        if not sections and len(questions) > 1:
            ...
            bullet_items = re.findall(r'[-•]\s*([^\n]+(?:\n(?![-•])[^\n]*)*)', answers, re.MULTILINE | re.DOTALL)
            ...
            if bullet_items and len(bullet_items) >= len(questions):
                for i, content in enumerate(bullet_items):
                    if i < len(questions):
                        ...
                        section_match = re.search(r'Section\s+(\d+[a-f]):', questions[i])
                        if section_match:
            ...
            for question in questions:
                ...
                for result in results:
                    ...
                    if section_key in result.get('analysis', {}):
                        section_answers = result['analysis'][section_key].get('answers', '')
                        parsed_answers = self._parse_section_answers(section_answers, questions)
                        ...
                        if question in parsed_answers:
                            ...
                            for quote in quotes:
                                clean_answer_text = clean_answer_text.replace(f'Quote: "{quote}"', '')
                                clean_answer_text = clean_answer_text.replace(f'Quote: {quote}', '')
                                clean_answer_text = clean_answer_text.replace(f'- Quote: "{quote}"', '')
                                clean_answer_text = clean_answer_text.replace(f'- Quote: {quote}', '')
                                clean_answer_text = clean_answer_text.replace(f'**Quote:** "{quote}"', '')
                            ...
                            for existing in consolidated["questions_by_section"][section_title][question]:
                            ...
                            if existing_answer:
                                ...
                                if len(clean_answer_text) > len(existing_answer['answer']):
                                ...
                                # Add new answer
                                consolidated["questions_by_section"][section_title][question].append({
    ...
    def export_to_text(self, results: List[Dict[str, Any]], output_file: str = None):
        ...
        try:
            ...
            with open(output_file, 'w', encoding='utf-8') as f:
                ...
                for citation, overview_data in consolidated["company_overview_by_expert"].items():
                ...
                for section_title, questions_data in consolidated["questions_by_section"].items():
                    f.write(f"\n{section_title.upper()}\n")
                    f.write("=" * 60 + "\n\n")
    ...
    def export_to_word(self, results: List[Dict[str, Any]], output_file: str = None):
        ...
        if not DOCX_AVAILABLE:
            ...
            return self.export_to_text(results, output_file.replace('.docx', '_fallback.txt'))
        ...
        try:
            ...
            # Add title
            title_para = doc.add_paragraph('FlexXray Transcript Analysis - Simplified Structure')
            ...
            summary_para = doc.add_paragraph(f'Analysis completed for {len(results)} transcripts.')
            ...
            # Section 1: Company Overview by Expert
            doc.add_page_break()
            section1_header = doc.add_paragraph('SECTION 1: COMPANY & INTERVIEWEE OVERVIEW (BY EXPERT)')
            ...
            for section_title, questions_data in consolidated["questions_by_section"].items():
                ...
                for question, responses in questions_data.items():
                    ...
                    if responses:
                        ...
                        no_response_para = doc.add_paragraph("• No responses found for this question.")
    ...
    def create_summarized_analysis(self, results: List[Dict[str, Any]], output_file: str = None) -> bool:
        if output_file is None:
            if CONFIG_AVAILABLE:
                output_file = get_output_path(OUTPUT_FILES["transcript_analysis_summary_txt"])
            else:
                ...
                for question, responses in questions_data.items():
                    if responses:
                        ...
                        for response in responses:
                            ...
                            question_content.append(f"• {answer_text} (Expert: {citation})")
...
"""
                        ...
                        try:
                            response = self.client.chat.completions.create(
                                model="gpt-4",
                                messages=[
                                    {"role": "system", "content": "You are an expert business analyst specializing in summarizing and synthesizing information from multiple sources. Focus on identifying patterns, themes, and actionable insights."},
                            ...
                            summary_sections.append(f"\nQuestion: {question}")
                            summary_sections.append("-" * 30)
                            summary_sections.append("Summary generation failed. Raw responses:")
                            for response in responses:
                                answer_text = response['answer']
                                citation = response['citation']
                                summary_sections.append(f"• {answer_text} (Expert: {citation})")
                    else:
                        summary_sections.append(f"\nQuestion: {question}")
                        summary_sections.append("-" * 30)
                        summary_sections.append("• No responses found for this question.")
    ...
    def create_summarized_analysis_word(self, results: List[Dict[str, Any]], output_file: str = None) -> bool:
        if output_file is None:
            if CONFIG_AVAILABLE:
                output_file = get_output_path(OUTPUT_FILES["transcript_analysis_summary_docx"])
            else:
        ...
        if not DOCX_AVAILABLE:
            print("python-docx not available, saving as text instead...")
            return self.create_summarized_analysis(results, output_file.replace('.docx', '.txt'))
        ...
        try:
            ...
            # Add title
            title_para = doc.add_paragraph('FlexXray Transcript Analysis - AI Summarized')
            title_para.alignment = 1
            if title_para.runs:
                title_run = title_para.runs[0]
                title_run.font.size = Inches(0.22)
...
"""
                        ...
                        try:
                            ...
                            for line in summary_lines:
                                if line.strip():
                                    if line.startswith('##'):
                                        # Section header
                                        section_para = doc.add_paragraph(line.replace('##', '').strip())
                            ...
                            error_para = doc.add_paragraph("Summary generation failed. Raw responses:")
                            if error_para.runs:
                            ...
                            for response in responses:
                                answer_text = response['answer']
                                citation = response['citation']
                                response_para = doc.add_paragraph(f"• {answer_text} (Expert: {citation})")
                                if response_para.runs:
                                    response_run = response_para.runs[0]
                                    response_run.font.name = 'Calibri'
                                    response_run.font.size = Inches(0.15)
            ...
            print("Saving as text instead...")
            return self.create_summarized_analysis(results, output_file.replace('.docx', '_fallback.txt'))

    def semantic_search_across_transcripts(self, query: str, top_k: int = 10, filter_by_type: str = None) -> List[Dict[str, Any]]:
    ...
    def create_concept_attribution_analysis(self, results: List[Dict[str, Any]], output_file: str = None) -> bool:
        if output_file is None:
            if CONFIG_AVAILABLE:
                output_file = get_output_path(OUTPUT_FILES["concept_attribution_analysis_txt"])
            else:
        ...
        try:
            ...
            with open(output_file, 'w', encoding='utf-8') as f:
                f.write("FLEXXRAY TRANSCRIPT ANALYSIS - CONCEPT ATTRIBUTION BY EXPERT\n")
                f.write("=" * 70 + "\n\n")
                f.write(f"Analysis completed for {len(results)} transcripts.\n")
                f.write(f"Analysis Date: {time.strftime('%Y-%m-%d %H:%M:%S')}\n")
                f.write(f"Vector database chunks: {len(concept_attribution)} concepts identified\n\n")
                
                # Section 1: Expert Overview with Key Contributions
                f.write("SECTION 1: EXPERT OVERVIEW & KEY CONTRIBUTIONS\n")
                f.write("=" * 70 + "\n\n")
                
                expert_contributions = self._analyze_expert_contributions(concept_attribution)
                for expert, contributions in expert_contributions.items():
                    ...
                    f.write(f"Primary Focus Areas: {', '.join(contributions['focus_areas'])}\n")
                    f.write(f"Notable Insights: {contributions['notable_insights']}\n\n")
                ...
                for theme, concepts in themed_concepts.items():
                    ...
                    for concept, attribution_data in concepts.items():
                        ...
                        for expert, details in attribution_data['experts'].items():
                            f.write(f"  • {expert}:\n")
                            f.write(f"    - Relevance Score: {details['relevance']:.3f}\n")
                            f.write(f"    - Key Quote: \"{details['key_quote']}\"\n")
                            f.write(f"    - Context: {details['context']}\n")
                        
                        f.write(f"  Overall Consensus: {attribution_data['consensus_level']}\n")
                        f.write(f"  Key Insights: {attribution_data['key_insights']}\n\n")
                
                # Section 3: Cross-Expert Analysis
                f.write("SECTION 3: CROSS-EXPERT ANALYSIS & CONSENSUS\n")
                f.write("=" * 70 + "\n\n")
                
                cross_expert_insights = self._analyze_cross_expert_patterns(concept_attribution)
        ...
        try:
            ...
            for i, chunk_text in enumerate(all_chunks['documents']):
                ...
                for concept in concepts:
                    ...
                    if relevance > concept_map[concept]['experts'][expert_name]['relevance'] * 0.8:
                        concept_map[concept]['experts'][expert_name]['key_quote'] = self._extract_key_quote(chunk_text, concept)
                        concept_map[concept]['experts'][expert_name]['context'] = self._extract_context(chunk_text, concept)
    ...
    def _extract_concepts_from_chunk(self, chunk_text: str, chunk_type: str) -> List[str]:
        ...
        concept_patterns = {
            'financial': [
                'revenue growth', 'profitability', 'cost structure', 'pricing strategy',
                'financial performance', 'investment', 'funding', 'cash flow'
            ],
            ...
            'risk_challenge': [
                'risk factors', 'challenges', 'threats', 'obstacles',
                'regulatory issues', 'market risks'
            ],
            'growth_strategy': [
                'growth opportunities', 'expansion plans', 'market expansion',
                'product development', 'strategic initiatives'
            ],
            'customer_market': [
                'customer needs', 'market demand', 'customer satisfaction',
                'market trends', 'customer relationships'
            ],
            'technology_product': [
                'technology innovation', 'product features', 'R&D',
        ...
        flexxray_concepts = [
            'foreign material detection', 'x-ray technology', 'food safety',
            'quality control', 'contamination prevention', 'inspection services'
        ]
        ...
        for company in company_patterns[:3]:
            if len(company.split()) <= 3 and company.lower() not in ["the", "and", "for", "with", "flexxray"]:
    ...
    def _extract_key_quote(self, chunk_text: str, concept: str) -> str:
        ...
        if sentences:
            return sentences[0].strip()[:150] + "..." if len(sentences[0]) > 150 else sentences[0].strip()
    ...
    def _analyze_expert_contributions(self, concept_map: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for concept, data in concept_map.items():
            for expert_name, expert_data in data['experts'].items():
                ...
                # Count chunks
                expert_analysis[expert_name]['chunk_count'] += len(expert_data['chunks'])
        ...
        for expert in expert_analysis:
            expert_analysis[expert]['key_topics'] = list(expert_analysis[expert]['key_topics'])
            expert_analysis[expert]['focus_areas'] = list(expert_analysis[expert]['focus_areas'])
            expert_analysis[expert]['notable_insights'] = expert_analysis[expert]['notable_insights'][:5]  # Top 5
        
        return expert_analysis
    
    def _group_concepts_by_theme(self, concept_map: Dict[str, Any]) -> Dict[str, Dict[str, Any]]:
        """Group concepts by business theme for better organization."""
        theme_mapping = {
            'financial_performance': ['revenue', 'profit', 'cost', 'financial', 'investment'],
            'competitive_position': ['competitive', 'advantage', 'strength', 'weakness', 'market position'],
            'growth_opportunities': ['growth', 'expansion', 'opportunity', 'development'],
        ...
        for concept, data in concept_map.items():
            ...
            for theme, keywords in theme_mapping.items():
                match_score = sum(1 for keyword in keywords if keyword in concept.lower())
    ...
    def _extract_key_insights(self, concept_data: Dict[str, Any]) -> str:
        ...
        high_relevance_experts = [
            expert for expert, data in concept_data['experts'].items()
            if data['relevance'] > 0.7
        ]
        
        if high_relevance_experts:
            insights.append(f"Strong insights from {len(high_relevance_experts)} experts")
        
        # Check for agreement/disagreement
        if len(concept_data['experts']) > 1:
            avg_relevance = sum(data['relevance'] for data in concept_data['experts'].values()) / len(concept_data['experts'])
    ...
    def _analyze_cross_expert_patterns(self, concept_map: Dict[str, Any]) -> Dict[str, List[str]]:
        ...
        for concept, data in concept_map.items():
            ...
            if len(experts) == 1:
                patterns['unique_perspectives'].append(f"{concept}: Only mentioned by {experts[0]}")
            elif len(experts) >= 3:
                # Check for consensus
                relevances = [data['experts'][expert]['relevance'] for expert in experts]
                avg_relevance = sum(relevances) / len(relevances)
                
                if avg_relevance > 0.6:
                    patterns['consensus_areas'].append(f"{concept}: Strong agreement among {len(experts)} experts")
                elif avg_relevance < 0.4:
                    patterns['disagreement_areas'].append(f"{concept}: Mixed opinions among {len(experts)} experts")
                else:
                    patterns['complementary_insights'].append(f"{concept}: Complementary perspectives from {len(experts)} experts")
        
        return patterns
    
    def _create_expert_agreement_matrix(self, concept_map: Dict[str, Any]) -> Dict[str, Dict[str, Dict[str, float]]]:
                ...
                for j, expert2 in enumerate(experts):
                    if i == j:
                        ...
                        agreement = self._calculate_expert_agreement(
                            data['experts'][expert1], 
                            data['experts'][expert2]
                        )
                        agreement_matrix[concept][expert1][expert2] = agreement
        
        return agreement_matrix
    
    def _calculate_expert_agreement(self, expert1_data: Dict[str, Any], expert2_data: Dict[str, Any]) -> float:
    ...
    def create_concept_attribution_analysis_word(self, results: List[Dict[str, Any]], output_file: str = None) -> bool:
        if output_file is None:
            if CONFIG_AVAILABLE:
                output_file = get_output_path(OUTPUT_FILES["concept_attribution_analysis_docx"])
            else:
                output_file = "FlexXray_concept_attribution_analysis.docx"
        """Create a Word document version of the concept attribution analysis."""
        if not DOCX_AVAILABLE:
            print("python-docx not available, saving as text instead...")
            return self.create_concept_attribution_analysis(results, output_file.replace('.docx', '.txt'))
        ...
        try:
            ...
            # Add title
            title_para = doc.add_paragraph('FlexXray Transcript Analysis - Concept Attribution by Expert')
            title_para.alignment = 1
            if title_para.runs:
                title_run = title_para.runs[0]
                title_run.font.size = Inches(0.22)
            ...
            # Section 1: Expert Overview with Key Contributions
            doc.add_page_break()
            section1_header = doc.add_paragraph('SECTION 1: EXPERT OVERVIEW & KEY CONTRIBUTIONS')
            if section1_header.runs:
                section1_header_run = section1_header.runs[0]
                section1_header_run.font.bold = True
                section1_header_run.font.size = Inches(0.24)
            ...
            for expert, contributions in expert_contributions.items():
                ...
                details_text += f"Primary Focus Areas: {', '.join(contributions['focus_areas'])}\n"
                details_text += f"Notable Insights: {', '.join(contributions['notable_insights'])}"
            ...
            # Section 2: Concept-by-Concept Attribution
            doc.add_page_break()
            section2_header = doc.add_paragraph('SECTION 2: CONCEPT ATTRIBUTION BY BUSINESS THEME')
            ...
            for theme, concepts in themed_concepts.items():
                ...
                for concept, attribution_data in concepts.items():
                    ...
                    consensus_para = doc.add_paragraph(f"  Overall Consensus: {attribution_data['consensus_level']}")
                    ...
                    insights_para = doc.add_paragraph(f"  Key Insights: {attribution_data['key_insights']}")
            ...
            # Section 3: Cross-Expert Analysis
            doc.add_page_break()
            section3_header = doc.add_paragraph('SECTION 3: CROSS-EXPERT ANALYSIS & CONSENSUS')
            ...
            # Section 4: Expert Agreement/Disagreement Matrix
            doc.add_page_break()
            section4_header = doc.add_paragraph('SECTION 4: EXPERT AGREEMENT/DISAGREEMENT MATRIX')
            ...
            matrix_header = doc.add_paragraph("Expert Agreement Levels (Higher = More Agreement):")
            ...
            print("Saving as text instead...")
            return self.create_concept_attribution_analysis(results, output_file.replace('.docx', '_fallback.txt'))

    def export_to_text_with_chunks(self, results: List[Dict[str, Any]], output_file: str = None) -> bool:
        if output_file is None:
            if CONFIG_AVAILABLE:
                output_file = get_output_path(OUTPUT_FILES["transcript_analysis_with_chunks_txt"])
            else:
        ...
        try:
            ...
            with open(output_file, 'w', encoding='utf-8') as f:
                ...
                f.write("SECTION 1: COMPANY & INTERVIEWEE OVERVIEW (BY EXPERT) - ENHANCED\n")
                f.write("=" * 70 + "\n\n")
                    ...
                    for question, responses in questions_data.items():
                        ...
                        if responses:
                            for response in responses:
                                ...
                                expert_chunks = self._get_expert_chunks(results, citation)
                                relevant_chunks = self._find_relevant_chunks_for_question(expert_chunks, question)
                                
                                if relevant_chunks:
                                    f.write(f"\n  Supporting Chunk Context:\n")
                                    for chunk in relevant_chunks[:2]:  # Show top 2 relevant chunks
                                        f.write(f"    - {chunk['text'][:120]}...\n")
                                        f.write(f"      (Chunk Type: {chunk['metadata'].get('chunk_type', 'Unknown')})\n")
                ...
                f.write(f"Average Chunks per Transcript: {total_chunks / len(results):.1f}\n\n")
                ...
                for chunk_type, count in chunk_type_distribution.items():
                    ...
                    f.write(f"  {chunk_type.replace('_', ' ').title()}: {count} chunks ({percentage:.1f}%)\n")
                ...
                for expert_pair, similarity_data in similarities.items():
                    f.write(f"\n{expert_pair}:\n")
                    f.write(f"  Similarity Score: {similarity_data['similarity']:.3f}\n")
                    f.write(f"  Common Topics: {', '.join(similarity_data['common_topics'][:3])}\n")
                    f.write(f"  Shared Insights: {similarity_data['shared_insights']}\n")
    ...
    def _get_expert_chunks(self, results: List[Dict[str, Any]], expert_citation: str) -> List[Dict[str, Any]]:
        """Get chunks for a specific expert from the results."""
        for result in results:
            citation = self._extract_citation_enhanced(result.get('transcript_name', ''), result.get('doc_path'))
    ...
    def _find_relevant_chunks_for_question(self, chunks: List[Dict[str, Any]], question: str) -> List[Dict[str, Any]]:
        ...
        return [chunk for chunk, score in scored_chunks if score > 0.1][:3]  # Top 3 relevant chunks
    ...
    def _analyze_expert_chunk_contributions(self, results: List[Dict[str, Any]]) -> Dict[str, int]:
    ...
    def _identify_common_themes_across_chunks(self, results: List[Dict[str, Any]]) -> Dict[str, Dict[str, Any]]:
        ...
        for result in results:
            ...
            for chunk in chunks:
                ...
                if len(chunk_text) > len(theme_data[chunk_type]['key_insight']):
                    theme_data[chunk_type]['key_insight'] = chunk_text[:200] + "..." if len(chunk_text) > 200 else chunk_text
    ...
    def _analyze_cross_expert_chunk_similarities(self, results: List[Dict[str, Any]]) -> Dict[str, Dict[str, Any]]:
        ...
        for i, expert1 in enumerate(expert_citations):
            for j, expert2 in enumerate(expert_citations[i+1:], i+1):
                ...
                similarity_score = self._calculate_expert_chunk_similarity(expert1_chunks, expert2_chunks)
                common_topics = self._find_common_topics(expert1_chunks, expert2_chunks)
                shared_insights = self._identify_shared_insights(expert1_chunks, expert2_chunks)
    ...
    def _calculate_expert_chunk_similarity(self, chunks1: List[Dict[str, Any]], chunks2: List[Dict[str, Any]]) -> float:
        """Calculate similarity between two sets of chunks."""
        if not chunks1 or not chunks2:
            return 0.0
        
        # Get chunk types for each expert
        types1 = [chunk.get('metadata', {}).get('chunk_type', 'general') for chunk in chunks1]
        types2 = [chunk.get('metadata', {}).get('chunk_type', 'general') for chunk in chunks2]
    ...
    def _find_common_topics(self, chunks1: List[Dict[str, Any]], chunks2: List[Dict[str, Any]]) -> List[str]:
    ...
    def _identify_shared_insights(self, chunks1: List[Dict[str, Any]], chunks2: List[Dict[str, Any]]) -> str:
        ...
        types1 = set(chunk.get('metadata', {}).get('chunk_type', 'general') for chunk in chunks1)
        types2 = set(chunk.get('metadata', {}).get('chunk_type', 'general') for chunk in chunks2)
    ...
    def _generate_chunk_based_recommendations(self, results: List[Dict[str, Any]]) -> Dict[str, List[str]]:
        ...
        if total_chunks > 0:
            ...
            dominant_types = [k for k, v in chunk_type_distribution.items() if v > total_chunks * 0.2]
            if dominant_types:
                recommendations['focus_areas'].append(f"Strong focus on: {', '.join(dominant_types)}")
            
            underrepresented_types = [k for k, v in chunk_type_distribution.items() if v < total_chunks * 0.1]
            if underrepresented_types:
                recommendations['focus_areas'].append(f"Consider expanding coverage of: {', '.join(underrepresented_types)}")
            
            # Expert utilization recommendations
            expert_chunk_counts = self._analyze_expert_chunk_contributions(results)
            avg_chunks = total_chunks / len(expert_chunk_counts) if expert_chunk_counts else 0
            
            high_contributors = [k for k, v in expert_chunk_counts.items() if v > avg_chunks * 1.5]
            if high_contributors:
                recommendations['expert_utilization'].append(f"High contributors: {', '.join(high_contributors)} - leverage their insights")
            
            low_contributors = [k for k, v in expert_chunk_counts.items() if v < avg_chunks * 0.5]
            if low_contributors:
                recommendations['expert_utilization'].append(f"Low contributors: {', '.join(low_contributors)} - may need additional engagement")
            
            # Strategic opportunities
            if 'growth_strategy' in chunk_type_distribution and chunk_type_distribution['growth_strategy'] > total_chunks * 0.15:
                recommendations['strategic_opportunities'].append("Strong growth focus identified - consider expansion planning")
            
            if 'risk_challenge' in chunk_type_distribution and chunk_type_distribution['risk_challenge'] > total_chunks * 0.2:
                recommendations['strategic_opportunities'].append("Significant risk factors identified - prioritize risk mitigation")
        
        return recommendations

    def export_to_word_with_chunks(self, results: List[Dict[str, Any]], output_file: str = None) -> bool:
        if output_file is None:
            if CONFIG_AVAILABLE:
                output_file = get_output_path(OUTPUT_FILES["transcript_analysis_with_chunks_docx"])
            else:
        ...
        if not DOCX_AVAILABLE:
            print("python-docx not available, saving as text instead...")
            return self.export_to_text_with_chunks(results, output_file.replace('.docx', '.txt'))
        ...
        try:
            ...
            # Add title
            title_para = doc.add_paragraph('FlexXray Transcript Analysis - Enhanced with Chunk Insights')
            title_para.alignment = 1
            if title_para.runs:
                title_run = title_para.runs[0]
                title_run.font.size = Inches(0.22)
            ...
            # Section 1: Company Overview by Expert (Enhanced with chunks)
            doc.add_page_break()
            section1_header = doc.add_paragraph('SECTION 1: COMPANY & INTERVIEWEE OVERVIEW - ENHANCED')
            if section1_header.runs:
                section1_header_run = section1_header.runs[0]
                section1_header_run.font.bold = True
                section1_header_run.font.size = Inches(0.24)
            ...
            for citation, overview_data in consolidated["company_overview_by_expert"].items():
                ...
                if expert_chunks:
                    ...
                    for chunk_type, chunks in chunks_by_type.items():
                        ...
                        chunk_type_header = doc.add_paragraph(f"{chunk_type.replace('_', ' ').title()}:")
                        ...
                        for chunk in chunks[:3]:
                            chunk_text = chunk['text'][:150] + "..." if len(chunk['text']) > 150 else chunk['text']
            ...
            for section_title, questions_data in consolidated["questions_by_section"].items():
                doc.add_page_break()
                section2_header = doc.add_paragraph(f'{section_title.upper()} - ENHANCED WITH CHUNK CONTEXT')
                if section2_header.runs:
                    section2_header_run = section2_header.runs[0]
                    section2_header_run.font.bold = True
                    section2_header_run.font.size = Inches(0.24)
                ...
                for question, responses in questions_data.items():
                    ...
                    if responses:
                        for response in responses:
                            ...
                            # Add supporting quotes
                            for quote in quotes:
                                quote_para = doc.add_paragraph(f"  \"{quote}\" -- {citation}")
                                if quote_para.runs:
                                    quote_run = quote_para.runs[0]
                                    quote_run.font.name = 'Calibri'
                                    quote_run.font.size = Inches(0.13)
                            ...
                            if relevant_chunks:
                                chunk_context_heading = doc.add_paragraph("  Supporting Chunk Context:")
                                ...
                                for chunk in relevant_chunks[:2]:  # Show top 2 relevant chunks
                                    chunk_text = chunk['text'][:120] + "..." if len(chunk['text']) > 120 else chunk['text']
                                    chunk_context_para = doc.add_paragraph(f"    - {chunk_text}")
                                    if chunk_context_para.runs:
                                        chunk_context_run = chunk_context_para.runs[0]
                                        chunk_context_run.font.name = 'Calibri'
                                        chunk_context_run.font.size = Inches(0.12)
                                    
                                    chunk_type_para = doc.add_paragraph(f"      (Chunk Type: {chunk['metadata'].get('chunk_type', 'Unknown')})")
            ...
            total_chunks = sum(len(result.get('chunks', [])) for result in results)
            summary_stats_para = doc.add_paragraph(f"Total Chunks Created: {total_chunks}")
            ...
            avg_chunks_para = doc.add_paragraph(f"Average Chunks per Transcript: {total_chunks / len(results):.1f}")
            ...
            for chunk_type, count in chunk_type_distribution.items():
                percentage = (count / total_chunks) * 100 if total_chunks > 0 else 0
                chunk_type_para = doc.add_paragraph(f"  {chunk_type.replace('_', ' ').title()}: {count} chunks ({percentage:.1f}%)")
                if chunk_type_para.runs:
                    chunk_type_run = chunk_type_para.runs[0]
                    chunk_type_run.font.name = 'Calibri'
                    chunk_type_run.font.size = Inches(0.15)
            ...
            # Section 4: Semantic Relationships Between Chunks
            doc.add_page_break()
            section4_header = doc.add_paragraph('SECTION 4: SEMANTIC RELATIONSHIPS & INSIGHTS')
            if section4_header.runs:
                section4_header_run = section4_header.runs[0]
                section4_header_run.font.bold = True
                section4_header_run.font.size = Inches(0.24)
            ...
            # Find common themes across chunks
            common_themes_heading = doc.add_paragraph("Common Themes Across Transcripts:")
            ...
            for theme, details in common_themes.items():
                ...
                theme_details = doc.add_paragraph(f"  Mentioned by: {', '.join(details['experts'])}")
                ...
                theme_chunks = doc.add_paragraph(f"  Total chunks: {details['chunk_count']}")
                ...
                theme_insight = doc.add_paragraph(f"  Key insight: {details['key_insight']}")
            ...
            for expert_pair, similarity_data in similarities.items():
                ...
                similarity_score = doc.add_paragraph(f"  Similarity Score: {similarity_data['similarity']:.3f}")
                ...
                common_topics = doc.add_paragraph(f"  Common Topics: {', '.join(similarity_data['common_topics'][:3])}")
                ...
                shared_insights = doc.add_paragraph(f"  Shared Insights: {similarity_data['shared_insights']}")
            ...
            # Section 5: Chunk-Based Recommendations
            doc.add_page_break()
            section5_header = doc.add_paragraph('SECTION 5: CHUNK-BASED RECOMMENDATIONS')
            ...
            # Save the document
            print(f"Saving enhanced Word document with chunk insights to: {output_file}")
            doc.save(output_file)
            ...
            print("Saving as text instead...")
            return self.export_to_text_with_chunks(results, output_file.replace('.docx', '_fallback.txt'))
    ...
    def hierarchical_retrieval_with_reranking(
        ...
        """
        if not self.collection or not CHROMA_AVAILABLE:
            print("Warning: ChromaDB not available. Cannot perform hierarchical retrieval.")
            return []
        
        try:
            print(f"Performing hierarchical retrieval for query: '{query}'")
            print(f"Retrieving top {top_k_per_transcript} chunks per transcript, then reranking to top {final_top_k}")
            ...
            print(f"Found {len(transcript_names)} transcripts: {', '.join(transcript_names)}")
                ...
                if results['documents'] and results['documents'][0]:
                    ...
                    for i, doc in enumerate(results['documents'][0]):
                        chunk_data = {
                            "text": doc,
                            "metadata": results['metadatas'][0][i] if results['metadatas'] and results['metadatas'][0] else {},
                            "embedding_score": 1.0 - (results['distances'][0][i] if results['distances'] and results['distances'][0] else 0.0),
                            "transcript_name": transcript_name
                        }
            ...
            if use_llm_reranking and all_candidate_chunks:
                ...
                reranked_chunks = self._llm_rerank_chunks(query, all_candidate_chunks, final_top_k)
            else:
                # Simple reranking based on embedding scores
                print("Using embedding-based reranking...")
                reranked_chunks = self._embedding_based_rerank(all_candidate_chunks, final_top_k)
            ...
            print(f"Hierarchical retrieval complete. Returning top {len(final_results)} results.")
    ...
    def _llm_rerank_chunks(
        ...
        try:
            ...
            response = self.client.chat.completions.create(
                model="gpt-4o",
                messages=[
                    {"role": "system", "content": "You are a precise scoring system. Return only valid JSON arrays."},
            ...
            if not scores:
                print("Warning: Could not parse LLM scores, falling back to embedding-based reranking")
    ...
    def _extract_scores_from_llm_response(self, response: str, expected_count: int) -> List[float]:
        """
        ...
        try:
            ...
            score_pattern = re.findall(r'score[:\s]*(\d+\.?\d*)', response, re.IGNORECASE)
    ...
    def _embedding_based_rerank(self, chunks: List[Dict[str, Any]], final_top_k: int) -> List[Dict[str, Any]]:
        """
        Simple reranking based on embedding scores only.
        """
        # Sort by embedding score and return top K
        reranked_chunks = sorted(chunks, key=lambda x: x["embedding_score"], reverse=True)
        return reranked_chunks[:final_top_k]
    ...
    def enhanced_context_retrieval(
        ...
        if use_hierarchical:
            ...
            relevant_chunks = self.search_relevant_chunks(query, transcript_name, final_top_k)
        ...
        try:
            ...
            for result in results:
                ...
                relevant_chunks = self.search_relevant_chunks(
                    question, 
                    transcript_name, 
                    top_k=max_quotes_per_expert * 3  # Get more chunks to find better quotes
                )
                ...
                for chunk in relevant_chunks:
                    ...
                    potential_quotes = self._extract_potential_quotes(chunk_text, question)
                    
                    for quote in potential_quotes:
                        if len(quote) > 15 and len(quote) < 400:  # More lenient quote length
                            # Calculate quote relevance to the question
                            quote_relevance = self._calculate_quote_relevance(quote, question)
                            ...
                            all_expert_quotes.append({
                                ...
                                'source_chunk': chunk_text[:100] + '...' if len(chunk_text) > 100 else chunk_text
                            })
            
            if debug:
                print(f"Found {len(all_expert_quotes)} total quotes across all experts")
                print(f"Sample quotes: {[q['quote'][:50] + '...' for q in all_expert_quotes[:3]]}")
    ...
    def _extract_potential_quotes(self, text: str, question: str) -> List[str]:
        ...
        for sentence in sentences:
            ...
            if (len(sentence) > 15 and  # Reduced minimum length
                len(sentence) < 400 and  # Increased maximum length
                not sentence.startswith('Interviewer:') and
                not sentence.startswith('Interviewee:') and
                not sentence.startswith('Paul Rodriguez') and  # Skip interviewer names
                not sentence.startswith('Gaurav') and
                not sentence.startswith('Randy Jesberg') and
                not sentence.startswith('Cheryl Bertics') and
                not sentence.startswith('Eli Dantas') and
                not sentence.startswith('George Perry') and
                not sentence.startswith('Lee Reece') and
                not sentence.startswith('Peter Poteres') and
                # Look for meaningful content indicators
                any(keyword in sentence.lower() for keyword in [
                    'flexxray', 'company', 'business', 'market', 'customer', 'technology',
                    'service', 'quality', 'safety', 'detection', 'x-ray', 'inspection',
                    'competitive', 'advantage', 'growth', 'opportunity', 'risk', 'challenge',
                    'industry', 'trend', 'future', 'strategy', 'operation', 'process'
                ])):
        ...
        unique_quotes = list(dict.fromkeys(quotes))  # Preserve order while removing duplicates
    ...
    def _calculate_quote_relevance(self, quote: str, question: str) -> float:
        ...
        try:
            ...
            relevance_keywords = {
                'competitive': ['competitive', 'advantage', 'strength', 'benefit', 'superior', 'better', 'leader', 'market leader'],
                'growth': ['growth', 'opportunity', 'expand', 'market', 'future', 'potential', 'increase', 'development'],
                'risk': ['risk', 'challenge', 'problem', 'issue', 'concern', 'threat', 'obstacle', 'difficulty'],
                'technology': ['technology', 'x-ray', 'detection', 'equipment', 'innovation', 'capability', 'system'],
                'customer': ['customer', 'client', 'service', 'quality', 'satisfaction', 'experience', 'needs'],
                'financial': ['financial', 'revenue', 'cost', 'pricing', 'profit', 'investment', 'return', 'value']
            }
            ...
            keyword_matches = sum(1 for keyword in relevant_keywords if keyword in quote_lower)
...
"""
            ...
            response = self.client.chat.completions.create(
                model="gpt-4o",
                messages=[
                    {"role": "system", "content": "You are a quote selection expert. Return only the quote numbers or 'None'."},
            ...
            try:
                ...
                sorted_quotes = sorted(candidate_quotes, key=lambda x: x['relevance_score'], reverse=True)
...
def main():
    ...
    try:
        ...
        summarizer = TranscriptSummarizer(api_key, max_workers=3, chroma_persist_directory=chroma_dir)
        ...
        if os.path.exists(default_directory):
            ...
            directory_path = input("Enter the path to the directory containing Word document transcripts: ").strip()
        ...
        results = summarizer.process_transcripts_directory(directory_path, progress_callback)
        ...
        if results:
            ...
            print("- FlexXray_transcript_analysis_results.docx (simplified Word document)")
            print("- FlexXray_transcript_analysis_with_chunks.txt (enhanced with chunk insights)")
            print("- FlexXray_transcript_analysis_with_chunks.docx (enhanced Word document with chunk insights)")
            print("- FlexXray_transcript_analysis_summary.txt (AI-summarized analysis)")
            print("- FlexXray_transcript_analysis_summary.docx (AI-summarized Word document)")
            print("- FlexXray_concept_attribution_analysis.txt (concept attribution)")
            print("- FlexXray_concept_attribution_analysis.docx (concept attribution Word document)")
            ...
            if summarizer.collection:
                ...
                if "error" not in stats:
                    ...
                    print(f"- Chunk types: {', '.join([f'{k}: {v}' for k, v in stats['chunk_types'].items()])}")
                    print(f"- Chunk size: {stats['chunk_size_limit']} chars with {stats['overlap_size']} char overlap")
                ...
                for query in demo_queries:
                    ...
                    simple_results = summarizer.semantic_search_across_transcripts(query, top_k=3)
                    if simple_results:
                        for i, result in enumerate(simple_results[:2]):  # Show top 2 results
                            print(f"    Result {i+1} (Score: {result['similarity_score']:.3f}):")
                            print(f"      Source: {result['metadata'].get('transcript_name', 'Unknown')}")
                    ...
                    try:
                        hierarchical_results = summarizer.hierarchical_retrieval_with_reranking(
                            query, 
                        ...
                        if hierarchical_results:
                            for i, result in enumerate(hierarchical_results[:2]):
                                print(f"    Result {i+1} (Rank: {result.get('rank', 'N/A')}, "
                                      f"Final Score: {result.get('final_score', 'N/A'):.3f}):")
                                print(f"      Source: {result.get('transcript_name', 'Unknown')}")
