...
class InputValidator:
        ...
        if must_exist and not os.path.isdir(dir_path):
            if create_if_missing:
                try:
                    ...
                    raise InputValidationError(f"Cannot create directory {dir_path}: {e}")
            else:
    ...
    @staticmethod
    def validate_text_content(text: str, min_length: int = 1, max_length: Optional[int] = None) -> str:
        """Validate text content."""
        if not text or not isinstance(text, str):
            raise InputValidationError("Text must be a non-empty string")
        
        if len(text.strip()) < min_length:
            raise InputValidationError(f"Text must be at least {min_length} characters long")
        
        if max_length and len(text) > max_length:
            raise InputValidationError(f"Text must be no more than {max_length} characters long")
    ...
    def validate_quote_data(quote: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if quote['speaker_role'] not in valid_roles:
            raise QuoteValidationError(f"Invalid speaker role: {quote['speaker_role']}. Valid: {valid_roles}")
    ...
    def validate_perspective_data(perspective: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for field in required_fields:
            if field not in perspective:
                raise InputValidationError(f"Perspective missing required field: {field}")
        
        # Validate title
        if not isinstance(perspective['title'], str) or not perspective['title'].strip():
            raise InputValidationError("Perspective title must be a non-empty string")
        
        # Validate description
        if not isinstance(perspective['description'], str) or not perspective['description'].strip():
            raise InputValidationError("Perspective description must be a non-empty string")
        ...
        for i, focus_area in enumerate(perspective['focus_areas']):
            if not isinstance(focus_area, str) or not focus_area.strip():
                raise InputValidationError(f"Focus area at index {i} must be a non-empty string")
        
        return perspective
    
    @staticmethod
    def validate_search_parameters(query: str, n_results: int = 10, 
                                 filter_metadata: Optional[Dict[str, Any]] = None) -> tuple:
    ...
    @staticmethod
    def validate_model_parameters(model: str, temperature: float, max_tokens: int) -> tuple:
...
class DocumentValidator:
    ...
    def validate_document_format(file_path: str) -> str:
        ...
        return InputValidator.validate_file_path(file_path, must_exist=True, 
    ...
    def validate_transcript_name(name: str) -> str:
        ...
        if len(name.strip()) < 3:
            raise DocumentValidationError("Transcript name must be at least 3 characters long")
    ...
    def validate_prompt_config(config: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for section in required_sections:
            if section not in config:
                raise ConfigurationError(f"Prompt configuration missing required section: {section}")
    ...
    def validate_environment_config(config: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for key in required_keys:
            if key not in config or not config[key]:
                raise ConfigurationError(f"Environment configuration missing required key: {key}")
