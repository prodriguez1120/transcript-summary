...
class QuoteTopicFilter:
    ...
    def __init__(self, api_key: str = None, use_fuzzy: bool = True):
        ...
        if self.use_fuzzy and self.fuzzy_matcher:
            ...
            for topic, patterns in self.topic_patterns.items():
                ...
                plain_patterns = [re.sub(r'[^\w\s]', '', pattern) for pattern in patterns]
    ...
    def filter_quotes_by_topic(self, quotes: List[Dict[str, Any]], topic: str) -> List[Dict[str, Any]]:
        ...
        if self.use_fuzzy and self.fuzzy_matcher:
            return self.fuzzy_matcher.enhanced_topic_filtering(quotes, topic, self.expanded_patterns[topic])
    ...
    def filter_quotes_by_ai(self, quotes: List[Dict[str, Any]], topic: str, max_quotes: int = 5) -> List[Dict[str, Any]]:
...
Select only the most relevant quotes for {topic_description}. Focus on quotes with specific details, metrics, or concrete evidence."""
        ...
        try:
            ...
            selected_indices = [item['quote_index'] for item in result.get('selected_quotes', [])]
    ...
    def get_all_topic_quotes(self, quotes: List[Dict[str, Any]], use_ai: bool = True) -> Dict[str, List[Dict[str, Any]]]:
        """Get quotes for all topics needed for company summary."""
        topic_quotes = {}
        
        for topic in self.topic_patterns.keys():
            if use_ai and self.client:
                topic_quotes[topic] = self.filter_quotes_by_ai(quotes, topic, max_quotes=4)
            else:
                topic_quotes[topic] = self.filter_quotes_by_topic(quotes, topic)[:4]
                
        return topic_quotes
    
    def format_quotes_for_prompt(self, topic_quotes: Dict[str, List[Dict[str, Any]]]) -> Dict[str, str]:
        """Format quotes for the company summary prompt variables."""
        formatted_quotes = {}
        
        for topic, quotes in topic_quotes.items():
            if not quotes:
                formatted_quotes[f"{topic}_quotes"] = "No relevant quotes available for this topic."
                continue
    ...
    def get_company_summary_variables(self, quotes: List[Dict[str, Any]], use_ai: bool = True) -> Dict[str, str]:
