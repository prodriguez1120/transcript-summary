...
def expand_processing_scope():
    ...
    for perspective_key, perspective_data in perspectives.items():
        ...
        additional_quotes = _find_additional_relevant_quotes(
            expert_quotes, expanded_focus_areas, current_ranked, max_additional=100
        )
        ...
        processed_additional = _process_additional_quotes(
            additional_quotes, perspective_key, expanded_focus_areas
        )
...
def _expand_focus_areas(focus_areas):
    ...
    for area in focus_areas:
        ...
        if any(
            term in area_lower for term in ["business", "model", "position", "market"]
        ):
...
def _find_additional_relevant_quotes(
    expert_quotes, focus_areas, existing_ranked, max_additional=100
):
    ...
    available_quotes = [
        q for q in expert_quotes if q.get("text", "") not in existing_texts
    ]
...
def _process_additional_quotes(quotes, perspective_key, focus_areas):
    ...
    for quote in quotes:
        ...
        if focus_matches:
            ...
            processed_quote["relevance_score"] = (
                3.0  # Lower score for expanded processing
            )
